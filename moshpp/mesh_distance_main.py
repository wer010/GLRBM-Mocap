...
from mesh_distance import sample2meshdist
...
def sample_from_mesh(mesh, sample_type='edge_midpoints', num_samples=10000, vertex_indices_to_sample=None, seed=0):
    ...
    if sample_type == 'vertices':
        if vertex_indices_to_sample is None:
            ...
            JS = co3(sample_ind)
            VS = np.ones(IS.size)
            point2sample = sparse(IS.flatten(), JS.flatten(), VS.flatten(), 3 * sample_ind.size,
                                         3 * mesh.v.shape[0])
...
class MeshDistanceSquared(Ch):
    ...
    def direction(self):
        from ch_vert_normals import VertNormals, TriNormals
